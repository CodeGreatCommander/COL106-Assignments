...
void checkParseTreeBasic(ExprTreeNode *root, std::vector<std::string> &tokenVector, bool &error, bool isMainRoot = false){
    if(root == nullptr){
        error = true;
        std::cerr << "this line is error Parse tree isn't built correctly" << std::endl;
        return;
    }
    if(isMainRoot){
    ...
    if(root->left == nullptr || root->right == nullptr){
        error = 1;
        std::cerr << "2nd attempt Parse tree isn't built correctly" << std::endl;
        return;
    }
    ...
    for(int  fileNum = 1; fileNum <= NUM_FILES; fileNum++) {
        ...
        while (std::getline(inputFile, line)) {
            ...
            if(tokenVector != tokens || parseError){
                std::cerr << "Parse tree isn't built correctly for line number "<< lineNum <<" in test case " <<fileNum <<" with the information "<<(tokenVector != tokens)<<" "<<parseError<< std::endl;
